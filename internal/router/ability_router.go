...
func userRouter(r *gin.RouterGroup, gHanFun ...gin.HandlerFunc) {
	v1 := r.Group("/").Use(gHanFun...)
	{
		...
		v1.GET("/ability/loginlog", ginx.Handle(login.GetListByPage))
		...
		v1.GET("/ability/userbaseinfo", ginx.Handle(payouts.GetUserBaseInfo))
		v1.GET("/ability/userrechargelog", ginx.Handle(payouts.GetUserRechargeListByPage))
